...
const File2 = () => {
  ...
  useEffect(() => {
    if (loggedIn && username) {
      axios
        .get(`http://localhost:8000/files/file2/${username}`)
        .then((response) => {
  ...
  const confirmUploadHandler = () => {
    ...
    axios
      ...
      .then((response) => {
        const fileUrl = `http://localhost:8000/uploads/file2/${selectedFile.name}`;
        setFiles((prevFiles) =>
  ...
  const deleteFileHandler = (name, visibility) => {
    ...
    const encodedPageName = "file2"; 
  
    axios
      .delete(`http://localhost:8000/file2/upload/?page_name=${encodedPageName}&file_name=${encodedFileName}&user_id=${username}`)
      .then(() => {
  ...
  return (
    <>
      ...
      <div className="archive-details-container">
        <h1 className="archive-details-header">Study Resources: Year 2</h1>
