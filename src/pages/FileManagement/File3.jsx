...
const File3 = () => {
  ...
  useEffect(() => {
    if (loggedIn && username) {
      axios
        .get(`http://localhost:8000/files/file3/${username}`)
        .then((response) => {
  ...
  const confirmUploadHandler = () => {
    ...
    axios
      .post(`http://localhost:8000/file3/upload/`, formData)
      .then((response) => {
        const fileUrl = `http://localhost:8000/uploads/file3/${selectedFile.name}`;
        setFiles((prevFiles) =>
  ...
  const deleteFileHandler = (name, visibility) => {
    ...
    const encodedPageName = "file3"; 
  
    axios
      .delete(`http://localhost:8000/file3/upload/?page_name=${encodedPageName}&file_name=${encodedFileName}&user_id=${username}`)
      .then(() => {
  ...
  return (
    <>
      ...
      <div className="archive-details-container">
        <h1 className="archive-details-header">Study Resources: Year 3</h1>
