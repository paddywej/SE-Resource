...
const File4 = () => {
  ...
  useEffect(() => {
    if (loggedIn && username) {
      axios
        .get(`http://localhost:8000/files/file4/${username}`)
        .then((response) => {
  ...
  const confirmUploadHandler = () => {
    ...
    axios
      .post(`http://localhost:8000/file4/upload/`, formData)
      .then((response) => {
        const fileUrl = `http://localhost:8000/uploads/file4/${selectedFile.name}`;
        setFiles((prevFiles) =>
  ...
  const deleteFileHandler = (name, visibility) => {
    ...
    const encodedPageName = "file4"; 
  
    axios
      .delete(`http://localhost:8000/file4/upload/?page_name=${encodedPageName}&file_name=${encodedFileName}&user_id=${username}`)
      .then(() => {
  ...
  return (
    <>
      ...
      <div className="archive-details-container">
        <h1 className="archive-details-header">Study Resources: Year 4</h1>
