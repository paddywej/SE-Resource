...
const File5 = () => {
  ...
  useEffect(() => {
    if (loggedIn && username) {
      axios
        .get(`http://localhost:8000/files/file5/${username}`)
        .then((response) => {
  ...
  const confirmUploadHandler = () => {
    ...
    axios
      .post(`http://localhost:8000/file5/upload/`, formData)
      .then((response) => {
        const fileUrl = `http://localhost:8000/uploads/file5/${selectedFile.name}`;
        setFiles((prevFiles) =>
  ...
  const deleteFileHandler = (name, visibility) => {
    ...
    const encodedPageName = "file5"; 
  
    axios
      .delete(`http://localhost:8000/file5/upload/?page_name=${encodedPageName}&file_name=${encodedFileName}&user_id=${username}`)
      .then(() => {
  ...
  return (
    <>
      ...
      <div className="archive-details-container">
        <h1 className="archive-details-header">Projects: Year 1</h1>
