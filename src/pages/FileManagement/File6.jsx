...
const File6 = () => {
  ...
  useEffect(() => {
    if (loggedIn && username) {
      axios
        .get(`http://localhost:8000/files/file6/${username}`)
        .then((response) => {
  ...
  const confirmUploadHandler = () => {
    ...
    axios
      .post(`http://localhost:8000/file6/upload/`, formData)
      .then((response) => {
        const fileUrl = `http://localhost:8000/uploads/file6/${selectedFile.name}`;
        setFiles((prevFiles) =>
  ...
  const deleteFileHandler = (name, visibility) => {
    ...
    const encodedPageName = "file6"; 
  
    axios
      .delete(`http://localhost:8000/file6/upload/?page_name=${encodedPageName}&file_name=${encodedFileName}&user_id=${username}`)
      .then(() => {
  ...
  return (
    <>
      ...
      <div className="archive-details-container">
        <h1 className="archive-details-header">Projects: Year 2</h1>
